...
from squirrel.widgets import FlowLayout
...
class TagChip(QtWidgets.QFrame):
    ...
    **kwargs : Any
    ...
    def paintEvent(self, event):
        ...
        x_margin = max(0, (event.rect().width() - self.sizeHint().width()) // 2)
        y_margin = max(0, (event.rect().height() - self.sizeHint().height()) // 2)

        margins.setLeft(x_margin)
        margins.setRight(x_margin)
        margins.setTop(y_margin)
        margins.setBottom(y_margin)
    ...
    def paint(self, painter):
        ...
        pen.setColor(QtGui.QColor(squirrel.color.GREY))
        painter.setPen(pen)
        ...
        if self.isEnabled():
            if len(tag_strings) > 0:
                icon = qta.icon("ph.x-bold", color=squirrel.color.GREY)
            else:
                icon = qta.icon("ph.plus-bold", color=squirrel.color.GREY)
